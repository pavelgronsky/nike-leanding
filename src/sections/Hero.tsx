    ...
    <section className="max-container flex min-h-screen w-full flex-col justify-center gap-10  p-2 xl:flex-row">
      <div className="max-xl:padding-x relative flex w-full flex-col items-start justify-center pt-28 xl:w-2/5">
        ...
        <h1 className="mt-10 font-palanquin text-8xl font-bold max-sm:text-[72px] max-sm:leading-[82]">
          <span className="relative z-10 pr-10 xl:whitespace-nowrap xl:bg-white">The new Arrival </span>
          <br />
        ...
        <div className="mt-20 flex w-full flex-wrap items-start justify-start gap-16">
          {statistics.map((statisticItem) => (
            <div key={statisticItem.label}>
              <p className="font-palanquin text-4xl font-bold">{statisticItem.value}</p>
              <p className="font-montserrat leading-7 text-slate-gray">{statisticItem.label}</p>
            </div>
          ))}
        </div>
      </div>
